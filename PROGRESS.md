...
- [2026-07-21] Reactの`ErrorBoundary`を追加（`frontend/src/App.tsx`、`AuthenticatedApp`をラップ）。ユーザーの友人が別環境（ブラウザ/OS不明、調査中）で銘柄をクリックすると「画面が暗転する」と報告。ErrorBoundaryが無いと描画エラー時にReactツリー全体がアンマウントされ、ダークテーマのbody背景（ほぼ黒）だけが残るため症状と一致すると判断。描画エラーが起きてもエラーメッセージを表示し他の銘柄に戻れるようにする安全対策を先行実装。根本原因（どのデータ・ブラウザで再現するか）は友人からの詳細待ちで引き続き調査中
