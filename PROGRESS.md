...
## モデル実績（シグナル的中率）検証機能の追加（2026-07-21）

ユーザーから「過去のデータで実際どれだけ当たっているか検証できないか」との要望を受けて実装。

- 当初は2分割（train/valid）のvalid_dfをそのまま使って的中率を測ろうとしたが、ユーザーから「それは過学習・リークにならないか」と鋭い指摘を受け、精査したところ正しい懸念だった: valid_dfは既にearly stoppingとパーセンタイル較正の両方に使われており、同じデータで的中率を測ると甘く出る
- 対応: train_model.pyを**3分割**（学習70%／検証15%=early stopping+パーセンタイル較正専用／テスト15%=一切調整に使わない最新期間、的中率レポート専用）に変更
- テスト期間のデータに対し、本番と全く同じロジック（predictor.py）でシグナルを再現し、実際の株価変動と比較。買い/売り/中立シグナルそれぞれの件数・的中率（買いは実際プラスだった割合、売りは実際マイナスだった割合）・平均実リターンを`model/predictor_validation_report.json`として保存
- `backend/main.py`に`GET /api/model-validation`を追加、フロントのランキング画面に「モデルの実績（過去データでの検証）」カードとして表示
- backendテスト11件・フロントlint/buildとも成功確認済み

## 全体監査結果（2026-07-21実施、忘れないよう記録）

ユーザー依頼「現時点で挙げたファイル全部見て、予測方式やAIの学習がおかしいところがないか精査して」を受けて全ファイルを精査した結果一覧。対応状況は各項目末尾に記載（このセクション自体は監査記録として恒久的に残す）。

### 🔴 重大
1. **train_model.pyの検証データ分割が実質「日付ベース」になっていなかった**: 銘柄ごとにサンプルを作ってから225銘柄分を連結し、末尾20%を検証用にしていたが、これは「全銘柄の直近20%」ではなく「後ろの方の約45銘柄の全期間」になっていた。銘柄間の相関を通じて学習データに検証期間の情報が漏れる形になっており、検証MAEが実力より楽観的だった可能性がある。 → **対応: 全サンプルを日付でソートしてから分割するよう修正**
2. **自律巡回クローラーに同時実行数の上限がなかった**: `autonomous_core_crawler`/`autonomous_backtest_crawler`が`asyncio.create_task`で前のタスク完了を待たずに次を起動しており、1タスクの処理時間がループ間隔(30秒)を超えると処理が際限なく積み上がり、非力なVM（1/8 OCPU）でリソース枯渇を起こすリスクがあった。 → **対応: `create_task`をやめ`await`で逐次実行に変更、backtestクローラーの間隔も60秒に延長**

### 🟡 中程度
3. **バックテスト「計算中」表示がポーリングされず固まる**: コアスコアの「分析中」には自動再取得があるが、バックテストには無く、計算完了後もページ再読み込みしないと反映されなかった。 → **対応: フロントにバックテスト用のポーリングを追加**
4. **学習期間(3年)とバックテスト適用期間(5年)の不一致**: パーセンタイル較正が3年分の検証データから作られる一方、バックテストはその較正を5年分に適用しており、較正されていない古い2年分でスコアが偏る可能性があった。 → **対応: train_model.pyのHISTORY_PERIODを5年に拡張しbacktest_engine.pyと揃える**
5. **Geminiバッチ処理のJSON解析が脆弱**: 実際に本番ログで構文エラーが発生し、その場合バッチ内の全銘柄がエラー扱いになっていた。 → **対応: コードフェンス除去等の寛容なJSONパース処理を追加**
6. **チャート期間切り替えの日付比較がタイムゾーンずれ**: ラベル日付をISO文字列経由でUTC解釈していたが、カットオフ日時はローカルタイムゾーンで計算しており、境界で最大1日程度のずれが起きうた。 → **対応: ラベル日付もローカルタイムゾーンで構築するよう修正**

### 🟢 軽微
7. **risk_scoreフィールドが常に固定値50で、フロントでも一切未使用**（旧プロトタイプの残骸）。 → **対応: バックエンド・フロントエンド・DBスキーマから完全に削除**
8. **短期スコアがRSI・MACDのみで、計算済みのma_ratio/bb_percent_b等を未使用**。 → **対応: score_from_technicalsに軽い重み付けで組み込み**
9. **RSI計算で「下落日が皆無」の極端ケースだと理論値100のところ50（中立）になっていた**。 → **対応: features.pyのエッジケースを修正**
10. **docker-compose.yml/.env.example/Dockerfileに、廃止済みのCookie認証やCloudflare Pages、Ampere/ARM前提を示す古いコメントが残っていた**（動作への影響はなし）。 → **対応: コメントを実態に合わせて更新**
11. `EDINET_API_KEY`は設定のみでコード未使用（フェーズ3待ちのため想定通り、対応不要）

**2026-07-21: 上記1〜10すべて修正完了。backendテスト11件・フロントlint/buildとも成功確認済み。RSIエッジケース・短期スコア改善は個別に動作検証済み（純上昇トレンドでRSI=100、横ばいでRSI=50を確認）。修正後、`train_model.py`を再実行してモデル・パーセンタイル較正データを再生成しVMに反映する。**
...
- [2026-07-21] **重複ランキングの解消**: ユーザーから「買うべき銘柄」と「売るべきでない銘柄」が全項目同じで意味がないとの指摘。バックエンドの`get_recommendations`を確認したところ、`bottom_sell = top_buy`・`bottom_buy = top_sell`と、実装当初から同一リストを別ラベルで表示していただけだったと判明（4リストに分ける実質的な意味がなかった）。`RankingResponse`から`bottom_buy`/`bottom_sell`を削除し、「買うべき銘柄TOP5」「売るべき銘柄TOP5」の2リストのみに簡素化。`FEATURES.md` 6節を修正
- [2026-07-21] ランキング一覧の各項目に証券コードだけでなく企業名も表示するよう変更。`AuthenticatedApp`が既に取得済みの銘柄マスター(`stocks`)を`DashboardView`にpropsで渡し、`code -> name_ja`のMapを作ってコードの下に表示する形にした（バックエンド変更は不要）
- [2026-07-21] 銘柄詳細画面からランキング画面に戻る手段がなかった問題を修正。ヘッダーに「ランキングに戻る」ボタンを追加（`current`が選択されている時のみ表示）し、ロゴ部分もクリックで同じ動作をするようにした。`setCurrent(null)`でランキング表示に戻す。フロントlint/buildとも成功確認済み
- [2026-07-21] ユーザー要望3点＋要件定義との照合を実施:
  - **検索バーの固定化**: `frontend/src/App.tsx`のサイドバー(`<aside>`)を、検索バー部分（`flexShrink: 0`）とスクロール可能な一覧部分（`flex:1, overflowY:auto`）に分離。以前は両方が同じスクロール領域に入っており、一覧を下までスクロールすると検索バーが見えなくなっていた
  - **短期・中期・長期タブ**: ランキング画面にタブを追加。短期=`short_score`（テクニカル指標）、長期=`long_score`（ML予測リターン）、中期=`final_score`（既存の50:50合成スコア）でランキングし直す。`backend/main.py`の`/api/recommendations`に`term`クエリパラメータを追加（`_RANKING_TERM_SCORE_KEY`辞書でソートキーを切り替え）。新たな中期専用モデルは持たず、既存の3スコアを使い分ける設計とした
  - **算出根拠時点の明示**: ランキング表示中の銘柄のうち最も更新が古いものの時刻を検出し、「◯◯時点のデータに基づいて算出しています」と表示するようにした（`updated_at`は"YYYY-MM-DD HH:MM:SS"形式の文字列で辞書式比較がそのまま時系列順になる性質を利用）
  - **要件定義との照合で発覚した不整合の修正**: `FEATURES.md`は「ランキングが最初の画面」を実装済みとしていたが、実際のコードは初回ロード時に日経225の先頭銘柄を自動選択しており、ランキング画面がほとんど表示されない状態になっていた（`useEffect`内の`setCurrent(res.data[0])`を削除して解消）。また`FEATURES.md` 7節が「スコア等はインメモリ保持のため再起動で消える」という、フェーズ1のDB移行で既に解消済みの古い記述のまま放置されていたため修正。8節にErrorBoundary、2節に検索バー固定化を追記
  - backendテスト11件・フロントlint/buildとも成功確認済み
- [2026-07-21] Reactの`ErrorBoundary`を追加（`frontend/src/App.tsx`、`AuthenticatedApp`をラップ）。ユーザーの友人が別環境で銘柄をクリックすると「画面が暗転する」と報告。ErrorBoundaryが無いと描画エラー時にReactツリー全体がアンマウントされ、ダークテーマのbody背景（ほぼ黒）だけが残るため症状と一致すると判断。描画エラーが起きてもエラーメッセージを表示し他の銘柄に戻れるようにする安全対策を先行実装
  - ErrorBoundary導入後、実際のエラーメッセージを確認できた:「Failed to execute 'removeChild' on 'Node': The node to be removed is not a child of this node.」。これはReactアプリでのクラッシュとして非常によく知られたパターンで、多くの場合ブラウザの自動翻訳機能（Google翻訳等）がReactの管理するDOMを外部から書き換えてしまい、Reactの再描画と衝突して発生する
  - `frontend/index.html`を修正: `lang="en"`だったのを実態に合わせ`lang="ja"`に修正（言語判定のミスマッチが自動翻訳の誤爆を誘発していた可能性）、`<html translate="no">`と`<meta name="google" content="notranslate">`を追加してページを翻訳対象から除外。あわせて`<title>`もプレースホルダーの`frontend`から正式名称に修正
  - 友人に自動翻訳が有効になっていなかったか確認中。この対策で再発しなければ原因確定
